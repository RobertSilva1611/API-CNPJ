...
def extrair_dados_json(dados):
    ...
    if not lista_ies_ativas:
        ...
        for ie in lista_ies_ativas:
            ...
            if num_ie:
                ies_consolidadas.append(f"{sigla_estado}: {num_ie}")
        ...
        linha["IEs Consolidadas"] = " | ".join(ies_consolidadas) if ies_consolidadas else "ISENTO"
...
def gerar_pdf_cnpj(dados):
    ...
    def add_box(title, content):
        if not content or content.strip() == "":
            content = "NÃO INFORMADO / INEXISTENTE"
    ...
    add_box("CÓDIGO E DESCRIÇÃO DAS ATIVIDADES ECONÔMICAS SECUNDÁRIAS", dados.get('Atividades Secundárias', ''))
                ...
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        with st.container(border=True):
                            ...
                            with st.expander("Ver Quadro de Sócios (QSA)"):
                                socios = dados_empresa.get('Quadro de Sócios', '')
